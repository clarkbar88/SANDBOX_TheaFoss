...
impute_km <- function(f,y=conc,nd=nd,wt,tlab='Normal',...) {
...
# handle case of all detects
  if (no.nd) {
    ...
    f.pp <- f.pp |> mutate(tf.lab=tlab,yhat=!!y,yt=trans(!!y,tlab),yt.hat=trans(yhat,tlab),qd=qnorm(pp)) |> select(-yv)
...
# handle edge cases; pass through Monte Carlo imputations from <pp_km>
    ...
    f.pp <- f.pp |> mutate(tf.lab=tlab,yhat=y.r,yt=trans(!!y,tlab),yt.hat=trans(yhat,tlab),qd=qnorm(pp)) |> select(-y.r)
...
# handle non-edge cases; fit censored normal for transformed detects depending on given transformation
    ...
    f.pp <- f.pp |> mutate(tf.lab=tlab,yhat=yv,yt=trans(yv,tlab),yt.hat=yt,qd=qnorm(pp)) |> select(-yv)
